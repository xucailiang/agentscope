...
OLLAMA_EMBEDDING_MODEL = os.getenv("OLLAMA_EMBEDDING_MODEL", "bge-large:latest")
OLLAMA_LLM_MODEL = os.getenv("OLLAMA_LLM_MODEL", "qwen3:4b")

# OpenAI Models (using SiliconFlow compatible models)
OPENAI_EMBEDDING_MODEL = "BAAI/bge-large-zh-v1.5"  # SiliconFlow embedding model (1024 dimensions)
...
SIMPLE_DOCUMENTS = [
    ...
    Document(
        id="simple_2",
        metadata=DocMetadata(
            content={"type": "text", "text": "Bob collaborates with Alice on transformer architecture research at OpenAI."},
            doc_id="simple_2",
            chunk_id=0,
            total_chunks=1,
        ),
...
async def cleanup_collection(graph_store: Neo4jGraphStore):
...
async def test_vector_only_mode(
    ...
    try:
        ...
        await knowledge.add_documents(SIMPLE_DOCUMENTS[:2])  # Use only 2 docs for speed
...
async def test_mixed_ollama_openai():
...
async def test_mixed_openai_ollama():
...
async def test_dashscope_models():
...
async def main():
    ...
    print(f"   DashScope API Key: {'✅ Set' if DASHSCOPE_API_KEY else '❌ Not Set'}")
    
    print("\n⚠️  Please ensure:")
    print("   1. Neo4j is running locally with GDS plugin")
    print("   2. Ollama is running locally with required models")
    print("   3. API keys are set in environment variables (if testing those providers)")
