    ...
    try:
        ...
        if result["status"] == "success":
            ...
            assert result["levels"] >= 0, \
                f"Invalid levels: {result['levels']}"
...
async def test_global_search_with_communities(
    ...
    docs = [
        Document(
            ...
            metadata=DocMetadata(
                content={"type": "text", "text": "OpenAI researches artificial intelligence and machine learning with focus on GPT models and language understanding."},
                doc_id="ai_1",
                chunk_id=0,
                total_chunks=1,
            ),
        ...
        Document(
            id="ai_2",
            metadata=DocMetadata(
                content={"type": "text", "text": "DeepMind advances AI through deep learning and reinforcement learning research including AlphaGo and protein folding."},
                doc_id="ai_2",
                chunk_id=0,
                total_chunks=1,
            ),
        ...
        Document(
            id="space_1",
            metadata=DocMetadata(
                content={"type": "text", "text": "SpaceX develops reusable rockets and spacecraft for Mars colonization and satellite deployment."},
                doc_id="space_1",
                chunk_id=0,
                total_chunks=1,
            ),
        ...
        Document(
            id="space_2",
            metadata=DocMetadata(
                content={"type": "text", "text": "NASA explores space through missions to Mars, Moon, and beyond with robotic and crewed spacecraft."},
                doc_id="space_2",
                chunk_id=0,
                total_chunks=1,
            ),
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            assert all(r.score is not None for r in global_results), \
                "All results should have scores"
            assert all(0 <= r.score <= 1 for r in global_results), \
...
async def test_community_summary_generation(
    ...
    try:
        ...
        if result["status"] == "success" and result["community_count"] > 0:
            ...
            comm_count = await wait_for_communities(graph_store, min_count=1, timeout=15)
    ...
    try:
        ...
        if result["status"] == "success" and result["community_count"] > 0:
            ...
            async with driver.session(database=graph_store.database) as session:
                ...
                assert count > 0, f"Expected communities to have embeddings, got {count}"
    except Exception as e:
        pytest.skip(f"Community detection requires Neo4j GDS plugin: {e}")
...
async def test_community_detection_with_many_entities(
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            print(f"✓ Detected {result['community_count']} communities with {result['levels']} levels")
    except Exception as e:
        pytest.skip(f"Community detection requires Neo4j GDS plugin: {e}")
