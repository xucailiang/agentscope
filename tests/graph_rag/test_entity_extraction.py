...
async def test_entity_extraction_verification(
    ...
    doc = Document(
        ...
        metadata=DocMetadata(
            content={"type": "text", "text": "Albert Einstein developed the theory of relativity at Princeton University."},
            doc_id="verify_1",
            chunk_id=0,
            total_chunks=1,
        ),
    ...
    entity_count = await wait_for_entities(graph_store, min_count=2, timeout=15)
...
async def test_entity_extraction_with_gleanings(
    ...
    doc = Document(
        id="glean_1",
        metadata=DocMetadata(
            content={"type": "text", "text": "Marie Curie, a physicist and chemist, conducted pioneering research on radioactivity at the Radium Institute in Paris, France, working with her husband Pierre Curie."},
            doc_id="glean_1",
            chunk_id=0,
            total_chunks=1,
        ),
    ...
    entity_count = await wait_for_entities(graph_store, min_count=3, timeout=20)
    
    # Should extract multiple entities (Marie, Pierre, Radium Institute, etc.)
    assert entity_count >= 3, f"Expected at least 3 entities with gleanings, got {entity_count}"
...
async def test_max_entities_limit(
    ...
    doc = Document(
        id="many_entities",
        metadata=DocMetadata(
            content={"type": "text", "text": "Apple, Microsoft, Google, Amazon, Meta, Tesla, Netflix, Intel, IBM, Oracle, Samsung, Nvidia are major technology companies."},
...
async def test_entity_types_configuration(
    ...
    doc = Document(
        id="typed_1",
        metadata=DocMetadata(
            content={"type": "text", "text": "Steve Jobs founded Apple in Cupertino, California."},
            doc_id="typed_1",
            chunk_id=0,
            total_chunks=1,
        ),
...
async def test_entity_embedding_generation(
    ...
    doc = Document(
        id="embed_test",
        metadata=DocMetadata(
            content={"type": "text", "text": "Leonardo da Vinci was an Italian Renaissance artist."},
            doc_id="embed_test",
            chunk_id=0,
            total_chunks=1,
        ),
    ...
    count = await wait_for_entity_embeddings(graph_store, min_count=1, timeout=15)
