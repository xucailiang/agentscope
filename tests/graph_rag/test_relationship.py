...
async def test_relationship_storage_verification(
    ...
    doc = Document(
        ...
        metadata=DocMetadata(
            content={"type": "text", "text": "Bill Gates founded Microsoft and serves as its chairman. He also worked with Paul Allen to create the company."},
            doc_id="rel_verify",
            chunk_id=0,
            total_chunks=1,
        ),
    ...
    async with driver.session(database=graph_store.database) as session:
        ...
        print(f"✓ Relationship extraction completed. Found {rel_count} relationships (LLM-dependent)")
...
async def test_relationship_types(
    ...
    docs = [
        Document(
            id="rel_type_1",
            metadata=DocMetadata(
                content={"type": "text", "text": "Ada Lovelace worked with Charles Babbage on the Analytical Engine."},
                doc_id="rel_type_1",
                chunk_id=0,
                total_chunks=1,
            ),
        ...
        Document(
            id="rel_type_2",
            metadata=DocMetadata(
                content={"type": "text", "text": "Cambridge University is located in Cambridge, England."},
                doc_id="rel_type_2",
                chunk_id=0,
                total_chunks=1,
            ),
...
async def test_multi_hop_graph_traversal(
    ...
    docs = [
        Document(
            id="hop_1",
            metadata=DocMetadata(
                content={"type": "text", "text": "Alice mentors Bob in machine learning research."},
                doc_id="hop_1",
                chunk_id=0,
                total_chunks=1,
            ),
        ...
        Document(
            id="hop_2",
            metadata=DocMetadata(
                content={"type": "text", "text": "Bob mentors Charlie in deep learning techniques."},
                doc_id="hop_2",
                chunk_id=0,
                total_chunks=1,
            ),
        ...
        Document(
            id="hop_3",
            metadata=DocMetadata(
                content={"type": "text", "text": "Charlie works on neural network optimization."},
                doc_id="hop_3",
                chunk_id=0,
                total_chunks=1,
            ),
...
async def test_relationship_with_entity_mentions(
    ...
    doc = Document(
        id="mentions_test",
        metadata=DocMetadata(
            content={"type": "text", "text": "Einstein worked at Princeton on theoretical physics."},
    ...
    entity_count = await wait_for_entities(graph_store, min_count=1, timeout=15)
    ...
    async with driver.session(database=graph_store.database) as session:
        ...
        assert mention_count >= 1, f"Expected at least 1 MENTIONS relationship, got {mention_count}"
...
async def test_complex_relationship_extraction(
    ...
    doc = Document(
        id="complex_rel",
        metadata=DocMetadata(
            content={"type": "text", "text": "Stanford University, founded in 1885 by Leland Stanford, is located in California and has produced numerous Nobel Prize winners including Paul Berg who won in Chemistry."},
            doc_id="complex_rel",
            chunk_id=0,
            total_chunks=1,
        ),
