...
from agentscope.exception import EntityExtractionError, GraphQueryError
...
class GraphKnowledgeBase(KnowledgeBase):
    ...
    def __init__(
        ...
        if (enable_entity_extraction or enable_relationship_extraction) and llm_model is None:
            raise ValueError(
    ...
    def _entity_config_defaults(self) -> None:
        ...
        self.entity_extraction_config.setdefault("generate_entity_embeddings", True)
    ...
    async def add_documents(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            logger.info(f"Adding {len(documents_with_embeddings)} documents to graph store")
            await self.graph_store.add(documents_with_embeddings)
            
            # Step 3 & 4: Extract entities and relationships if enabled
            if self.enable_entity_extraction:
                await self._process_entities_and_relationships(documents_with_embeddings)
            ...
            if self.enable_community_detection and not self._first_add_documents_called:
                ...
                logger.info("First add_documents call - triggering community detection in background")
        ...
        try:
            ...
            if search_mode == "vector":
                ...
                return await self._graph_search(
                    query_embedding, limit, score_threshold, **kwargs
                )
    ...
    async def _embed_entities(self, entities: list[Entity]) -> list[Entity]:
        ...
        """
        if not self.entity_extraction_config.get("generate_entity_embeddings", True):
            return entities
    ...
    async def _hybrid_search(
        ...
        **kwargs: Any,
        ...
        vector_results, graph_results = await asyncio.gather(
            ...
            self._graph_search(query_embedding, limit, score_threshold, **kwargs),
    ...
    async def _global_search(
        ...
        **kwargs: Any,
        ...
        if not communities:
            logger.warning("No communities found, falling back to vector search")
    ...
    async def _process_entities_and_relationships(
        ...
        if self.enable_relationship_extraction:
            ...
            relationships = await self._extract_relationships(documents, entities_with_embeddings)
    ...
    async def _extract_entities(self, documents: list[Document]) -> list[Entity]:
        ...
        if self.entity_extraction_config.get("enable_gleanings", False):
            gleanings_rounds = self.entity_extraction_config.get("gleanings_rounds", 2)
            for round_num in range(gleanings_rounds):
                logger.info(f"Gleanings round {round_num + 1}/{gleanings_rounds}")
        ...
        async def extract_from_doc(doc: Document) -> list[Entity]:
            async with semaphore:
                return await self._extract_entities_from_text(
                    ...
                    max_entities=self.entity_extraction_config.get("max_entities_per_chunk", 10),
                )
    ...
    async def _extract_entities_from_text(
        ...
        """
        # Build prompt
        entity_types_str = ", ".join(self.entity_extraction_config.get("entity_types", []))
...
JSON array:"""
    ...
    async def _gleanings_pass(
        ...
        existing_names_str = ", ".join(list(existing_names)[:20])  # Limit to 20 for prompt
...
JSON array:"""
    ...
    async def _extract_relationships(
        ...
        entity_names_str = ", ".join(entity_names[:50])  # Limit to 50 for prompt
...
JSON array:"""
        ...
        try:
            ...
            for data in rel_data:
                try:
                    ...
                    logger.warning(f"Invalid relationship data: {data}, error: {e}")
    ...
    async def detect_communities(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            communities_raw = await self._run_gds_community_detection(algo, **kwargs)
        ...
        try:
            async with driver.session(database=self.graph_store.database) as session:
                ...
                async for record in result:
                    ...
                    community_map[comm_id]["entity_names"].append(record["entity_name"])
                ...
                for comm_data in community_map.values():
                    ...
                    community.__dict__["_entity_descriptions"] = comm_data["entity_descriptions"]
    ...
    async def _generate_community_summary(self, community: Community) -> Community:
...
JSON object:"""
    ...
    def _generate_simple_summaries(self, communities: list[Community]) -> list[Community]:
    ...
    async def _batch_embed_communities(
        ...
        """
        logger.info(f"Generating embeddings for {len(communities)} communities")
        ...
        embeddings_ok = 0
        for i, (comm, embedding) in enumerate(zip(communities, response.embeddings)):
